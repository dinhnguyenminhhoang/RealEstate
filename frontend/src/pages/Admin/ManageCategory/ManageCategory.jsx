...
import CategoryForm from "../../../components/FormManage/CategoryForm/CategoryForm";
...
const ManageCategory = () => {
  ...
  const handleDelete = async (category) => {
    try {
      const res = await deleteCategoryApi(category);
      if (res.status === 200) {
        openNotification({
          type: "success",
          message: "Thông báo",
          description: `Category with ID ${category} deleted successfully!`,
        });
  ...
  const handleSave = async (values) => {
    try {
      let res;
      if (editCategory) {
        res = await admiEditCategoryApi(values, editCategory._id);
      } else {
